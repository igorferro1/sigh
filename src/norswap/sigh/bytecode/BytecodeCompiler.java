 ...
 * <ul>
 * <li>{@code Int}, {@code Float}, {@code Bool}: {@code long}, {@code double},
 * {@code boolean}
 * (which is manipulated as an integer by JVM instructions, but still has its
 * own representation
 * in type descriptors)</li>
 * <li>{@code String}: {@link String}</li>
 * <li>{@code null}: {@link norswap.sigh.bytecode.Null#INSTANCE}</li>
 * <li>Arrays: proper array type for the representation of the component. For
 * multi-dimension
 * arrays, object arrays (e.g. {@code Object[][]}).</li>
 * <li>Structs: A Java class encoding the structure, in the default package and
 * using the
 * structure's name.</li>
 * <li>TODO: Functions: a {@link MethodHandle} to the method that implements the
 * function in
 * bytecode.</li>
 * <li>Types: the {@link Class} object for the type representation. {@code Type}
 * itself (the
...
public class BytecodeCompiler {
    ...
    // TODO: simplify with
    // https://asm.ow2.io/javadoc/org/objectweb/asm/commons/GeneratorAdapter.html ?
    ...
    /*
     * Slash-separated binary class name for the class containing the emitted
     * bytecode for the source
     * unit.
     */
    private String containerName;

    /*
     * Class visitor for the class containing the emitted bytecode for the source
     * unit.
     */
    ...
    /**
     * The list of (class name, class writer) pairs for the classes representing the
     * structures
    ...
    /**
     * Compile the given source unit (given as its root AST node) into a class whose
     * (dot-separated)
    ...
    private Object funDecl(FunDeclarationNode node) {
        ...
        // NOTE: The current semantic analysis check guarantee that there is we
        // unconditionally
        // return. So we do not have to worry about instructions not followed by a
        // return.
        // The only exception is for void methods - so we always add a return at the end
        // in that
        // case. In the future, it might be good to check that nothing follows a return
        // in semantic
        // analysis.
    ...
    private Object binaryExpression(BinaryExpressionNode node) {
        ...
        switch (node.operator) {
            ...
            case EQUALITY:
                comparison(node.operator, IFEQ, IF_ICMPEQ, IF_ACMPEQ, left, right);
                break;
            case NOT_EQUALS:
                comparison(node.operator, IFNE, IF_ICMPNE, IF_ACMPNE, left, right);
                break;
            case GREATER:
                comparison(node.operator, IFGT, -1, -1, left, right);
                break;
            case LOWER:
                comparison(node.operator, IFLT, -1, -1, left, right);
                break;
            case GREATER_EQUAL:
                comparison(node.operator, IFGE, -1, -1, left, right);
                break;
            case LOWER_EQUAL:
                comparison(node.operator, IFLE, -1, -1, left, right);
                break;
    ...
    private void numOperation(int longOpcode, int doubleOpcode, Type left, Type right) {
        if (left instanceof IntType && right instanceof IntType) {
            ...
            // in this case, we've added a L2D instruction before the long operand
            // beforehand
            method.visitInsn(doubleOpcode);
        } else {
    ...
    private void comparison(
        ...
        if (left instanceof IntType && right instanceof IntType) {
            ...
            // If left is an Int, we've added a L2D instruction before the long operand
            // beforehand
    ...
    /**
     * Visit all argument nodes, adding implicit conversion based on the target
     * parameter type
     * if needed.
     */
    ...
    private Object reference(ReferenceNode node) {
        ...
        if (decl instanceof VarDeclarationNode || decl instanceof ParameterNode) {
            ...
            // NOTE: This is not used when the reference is part of a function call, the
            // resolution
            // is handled in #funCall.

            // TODO proper handling/representation of function object
            // For now I use a method handle. There is no way to use it in the language
            // however.
            ...
            switch (decl.name()) {
                ...
                case "print":
                    ...
                    break;
                case "true":
                    loadConstant(method, 1);
                    break;
                case "false":
                    loadConstant(method, 0);
                    break;
                case "null":
                    loadConstant(method, null);
                    break;
    ...
    private Object structDecl(StructDeclarationNode node) {
        ...
        Type[] paramTypes = node.fields.stream().map(f -> (Type) reactor.get(f, "type")).toArray(Type[]::new);
    ...
    /**
     * Returns the {@link org.objectweb.asm.Type ASM Type} for the {@code type}
     * attribute of the
     * given node.
     */
    ...
    /**
     * Return the JVM field descriptor for the given node, which must have a
     * {@code type} attribute.
     */
    ...
    /**
     * Declares a variable introduce by the given declaration (which must be a
     * {@link
     * VarDeclarationNode} or {@link ParameterNode}, and returns its index in its
     * JVM method
     * scope.
     */
    ...
    /**
     * See {@link #registerVariable(DeclarationNode)}. Takes the ASM type of the
     * declaration
    ...
    /**
     * Returns the variable index for the given reference, which must be a reference
     * to a variable.
     */
    ...
    /**
     * Implicitly converts the value at the top of the stack (of type {@code right})
     * to the type
     * {@code left} if compatible, in which case {@code left} is returned. Otherwise
     * returns {@code
     * right}.
     */
    ...
    /**
     * Like {@link #implicitConversion(Type, Type)}, using the type attributes of
     * the passed nodes.
     */
    ...
    /**
     * Calls the right dup instruction depending on {@code type}, the type of the
     * value at the top
     * of the stack.
     */
    ...
    /**
     * Calls the right dup_x2 ([x, y] => [y, x, y]) instruction depending on
     * {@code type}, the type
    ...
    /**
     * Calls the right dup_x2 ([x, y, z] => [z, x, y, z]) instruction depending on
     * {@code type}, the
    ...
    /**
     * Calls the right pop instruction depending on {@code type}, the type of the
     * value at the top
     * of the stack.
     */
